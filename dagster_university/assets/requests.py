...
def adhoc_request(config: AdhocRequestsConfig, database: DuckDBResource) -> None:
    # Splitting name of file to remove extension
    file_path = constants.REQUEST_DESTINATION_TEMPLATE_FILE_PATH.format(config.filename.split('.')[0])

    # Filter trips that didn't start in the specified borough
    # Aggregates data by the day of the week and hour the trip started
    query = f"""
      select
